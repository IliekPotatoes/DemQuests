...
//title screen
TopDownGame.Dragon = function(){};
var shotTimer = 0;
	var idle = 0;
var gameStart = false;
TopDownGame.Dragon.prototype = {
  ...
  create: function() {
    ...
    this.map = this.game.add.tilemap('dragonmap1');

    //the first parameter is the tileset name as specified in Tiled, the second is the key to the asset
    this.map.addTilesetImage('DRA_base', 'drake1');
...
//////////
	...
	window.tex = 'Toms';
	this.t = this.game.add.text(this.player.x-275, this.player.y-260, window.tex, { font: "10px Arial", fill: "#ffffff", align: "center" });
...
//this.enemies.forEach(function(enemy){    this.game.physics.arcade.moveToObject(this.enemy, {x: this.player.x, y:this.player.y},100, this);}, this);
    for (var i = 0; i < this.enemies.children.length; i++) { 
        this.game.physics.arcade.moveToObject(this.enemies.children[i] , {x: this.player.x, y:this.player.y}, Math.floor((Math.random() * 150) + 50));
